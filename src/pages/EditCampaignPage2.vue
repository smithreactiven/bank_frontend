  ...
  <v-container>
    <v-container>
      <div v-if="selected_image">
        <v-img width="100%" height="200px" :cover="true"  :src="selected_image">
        </v-img>
      </div>
      <div v-else>
        <v-img width="100%" height="200px" :cover="true"  :src="campaign.image">
        </v-img>
      </div>
      ...
      <v-card @click="redirectToRewards" rounded="lg" variant="flat" color="grey-darken-4">
        <v-card-item class="height-60">
          <v-row align="center">
            ...
            <!--            <v-col cols="auto">-->
            <!--              <v-img width="25px" height="25px" :src="reward_image">-->
            <!--              </v-img>-->
            <!--            </v-col>-->
            <v-spacer></v-spacer>
            <v-col cols="auto">
...
export default {
  ...
  mounted() {
    ...
    this.campaign.id = this.$route.query.campaign_id
  ...
  methods: {
    getCampaign(campaign_id) {
      axios.post('api/getCampaign', {campaign_id: campaign_id})
          .then(response => {
            ...
            // lscache.set("reward_amount", this.campaign.reward_amount)
            // lscache.set("reward_currency", this.campaign.reward_currency)
            this.reward_currency = this.campaign.reward_currency
            this.reward_amount = this.campaign.reward_amount

            this.campaign.url = `https://t.me/TestWACom_bot/campaign?startapp=${this.campaign.id}`
            this.getCampaignTaskGroups(campaign_id)
    ...
    getCampaignTaskGroups(campaign_id) {
      axios.post('api/getCampaignTaskGroups', {campaign_id: campaign_id})
          .then(response => {
    ...
    editCampaign() {
      if (!this.selected_image) {
        this.selected_image = this.campaign.image
      }
      else {
        this.selected_image = this.upload_images[0]
      }

      // if (this.upload_images[0].size > maxUploadSize) {
      ...
      if (!this.campaign.title) {
        ...
        this.campaign.finish_date = this.finish_date
        this.campaign.finish_time = this.finish_time
        ...
        data.append('media', this.selected_image);

        // this.addCampaignTaskGroups()
        axios.post('api/editCampaign', data).then(() => {
          window.Telegram.WebApp.MainButton.hideProgress()
          window.Telegram.WebApp.MainButton.hide()
          window.Telegram.WebApp.offEvent('mainButtonClicked')
          window.Telegram.WebApp.showAlert("Campaign successfully edit!")
    ...
    async fromUrlToFile(reward_image) {
      this.reward_image_file = await fetch(reward_image).then(r => r.blob()).then(blobFile => new File([blobFile], "fileNameGoesHere", { type: "image/png" }))
    },
    getCampaignHash() {
    ...
    onUploadImage(event) {
      ...
      if (selectedImage.size > maxUploadSize) {
        ...
        this.selected_image = this.getFileURL(this.upload_images[0])
      }
    },
    clearFinishDate() {
      this.campaign.finish_date = ""
    },
    clearFinishTime() {
      this.campaign.finish_time = ""
    },
