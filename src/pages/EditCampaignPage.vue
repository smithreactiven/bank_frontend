...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getCampaign() {
        axios.post('api/getCampaign', {campaign_id: this.campaign.id})
            .then(response => {
              this.campaign = response.data.campaign
              this.campaign_image = this.campaign.image
            })
            .catch(error => {
              console.error(error);
            });
      ...
      createCampaign() {
        if (!this.campaign_image) {
          ...
          axios.post('api/editCampaign', data).then(() => {
            window.Telegram.WebApp.showAlert("Campaign successfully edited!")
